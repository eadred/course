...
seqOptional (x :. xs) = x P.>>= f
                        where f x' = seqOptional xs P.>>= f'
                                where f' xs' = P.return (x' :. xs')
...
reverse ::
  List a
  -> List a
reverse = let f acc x = x :. acc in foldLeft f Nil

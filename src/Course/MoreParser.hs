...
tok ::
  Parser a
  -> Parser a
tok p = p >>= (\a -> spaces >>> valueParser a)
...
between ::
  ...
  -> Parser a
  -> Parser a
between o c a = let hasClose a' = c >>> valueParser a' in
                o >>> a >>= hasClose
...
hex ::
  Parser Char
hex = let fromOpt (Full a) = valueParser (chr a)
          fromOpt Empty = failed in
      (readHex <$> replicateA 4 (satisfy isHexDigit)) >>= fromOpt
...
eof ::
  Parser ()
eof = let parse' Nil = Result Nil ()
          parse' inp = ErrorResult (ExpectedEof inp) in
      P parse'
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll fs = let isAll c = and $ fs <*> pure c in
                satisfy isAll
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny fs = let isAny c = or $ fs <*> pure c in
                satisfy isAny
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma o c p = let sepByComma p' = sepby (tok p') (charTok ',') in
                          betweenCharTok o c $ sepByComma p

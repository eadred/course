...
toList (ListZipper (l:.ls) x r) = toList $ ListZipper ls l (x:.r)
...
findLeft p (ListZipper l x r) = case break p l of
                                (_, Nil) -> IsNotZ
                                (Nil, l':.ls) -> IsZ $ ListZipper ls l' (x:.r)
                                (nols, l':.ls) -> IsZ $ ListZipper ls l' (reverse nols ++ (x:.r))


-- prop> findLeft2 (const p) -<< fromList xs == IsNotZ
--
-- >>> findLeft2 (== 1) (zipper [2, 1] 3 [4, 5])
-- [] >1< [2,3,4,5]
--
-- >>> findLeft2 (== 6) (zipper [2, 1] 3 [4, 5])
-- ><
--
-- >>> findLeft2 (== 1) (zipper [2, 1] 1 [4, 5])
-- [] >1< [2,1,4,5]
--
-- >>> findLeft2 (== 1) (zipper [1, 2, 1] 3 [4, 5])
-- [2,1] >1< [3,4,5]
findLeft2 ::
  (a -> Bool)
  -> ListZipper a
  -> MaybeListZipper a
findLeft2 _ (ListZipper Nil _ _) = IsNotZ
findLeft2 p (ListZipper (l:.ls) x r) = let newZipper = ListZipper ls l (x:.r) in
                                       if p l then IsZ newZipper else findLeft2 p newZipper
...
findRight p (ListZipper l x (r:.rs)) = let newZipper = ListZipper (x:.l) r rs in
                                       if p r then IsZ newZipper else findRight p newZipper
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop z = case moveLeft z of
                 IsNotZ -> end z
                 IsZ z' -> z'
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop z = case moveRight z of
                 IsNotZ -> start z
                 IsZ z' -> z'
...
moveLeftN' n z | n < 0 = moveRightN' (-n) z
moveLeftN' n z =
  let moveLeftN'' _ 0 z' = Right z'
      moveLeftN'' sofar n' z' = case moveLeft z' of
                              IsNotZ -> Left sofar
                              IsZ z'' -> moveLeftN'' (sofar + 1) (n' - 1) z'' in
  moveLeftN'' 0 n z
...
moveRightN' n z | n < 0 = moveLeftN' (-n) z
moveRightN' n z =
  let moveRightN'' _ 0 z' = Right z'
      moveRightN'' sofar n' z' = case moveRight z' of
                              IsNotZ -> Left sofar
                              IsZ z'' -> moveRightN'' (sofar + 1) (n' - 1) z'' in
  moveRightN'' 0 n z
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft y (ListZipper ls x rs) = ListZipper (x:.ls) y rs
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight y (ListZipper ls x rs) = ListZipper ls y (x:.rs)
...
-- /Tip:/ Use `zipWith`
  (ListZipper lfs f rfs) <*> (ListZipper ls x rs) = let newLs = zipWith ($) lfs ls
                                                        newRs = zipWith ($) rfs rs
                                                    in ListZipper newLs (f x) newRs
...
instance Extend ListZipper where
  f <<= z = let unf move z' = (\z'' -> (f z'', z'')) <$> toOptional (move z')
                unf' move = unfoldr (unf move) z in
            ListZipper (unf' moveLeft) (f z) (unf' moveRight)
...
instance Traversable ListZipper where
  traverse f (ListZipper ls x rs) = let fls = traverse f ls
                                        frs = traverse f rs
                                        fx = f x in
                                        lift3 ListZipper fls fx frs

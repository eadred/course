...
character ::
  Parser Char
character = let parse' Nil = ErrorResult Failed
                parse' (c:.cs) = Result cs c in
            P parse'
...
--   * If this succeeds then try and run the second parser
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy f = let parseFn Nil = ErrorResult Failed
                parseFn (c:.cs) = if f c then Result cs c else ErrorResult Failed in
            P parseFn
...
natural ::
  Parser Int
natural = let doRead cs = case read cs of
...
-- | Return a parser that produces at least the given number of values off the given parser.
-- This parser fails if the given parser fails in the attempt to produce at least the given number of values.
--
-- >>> parse (atLeastThisMany 4 upper) "ABCDef"
-- Result >ef< "ABCD"
--
-- >>> parse (atLeastThisMany 4 upper) "ABCDEFgh"
-- Result >gh< "ABCDEF"
--
-- >>> isErrorResult (parse (atLeastThisMany 4 upper) "ABCdEf")
-- True
atLeastThisMany :: Int -> Parser a -> Parser (List a)
atLeastThisMany n p = let f cs = (cs ++) <$> list p in
                      thisMany n p >>= f
...
surnameParser ::
  Parser Chars
surnameParser = upper .: atLeastThisMany 5 lower
...
phoneParser ::
  Parser Chars
phoneParser = let hasFinalDigit cs = is '#' >>> valueParser cs in
              digit .: phoneBodyParser >>= hasFinalDigit
